...
**Do not invoke directly.** This is a dependency contract — all `dev-implementation` skills read this before operating.

## Scope

This contract covers artifacts and conventions that are **implementation-specific**.
Conventions shared with `dev-conceptualization` (semantic types, pipeline graph, plans format) live in `dev-shared/contracts/`.
