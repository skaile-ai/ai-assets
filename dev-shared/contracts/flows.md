...
the skill nodes inside each flow. Global agent config lives in `dev-shared/agent-config.json`.
...
{ "skill": "dev-conceptualization/skills/30_blueprint/cf_datamodel" }
```
...
| Domain | Path | Flows |
|---|---|---|
| Conceptualization | `dev-conceptualization/flows/` | `concept-only`, `prototype`, `cli-concept`, `reverse-engineer` |
| Implementation | `dev-implementation/flows/` | `standard`, `full`, `cli`, `prototype` |
| Quality | `dev-quality/flows/` | `audit`, `review`, `readiness` (add as needed) |
| Schema | `dev-shared/flow.schema.json` | JSON Schema for all flow files |
...
**Concept flows** (`dev-conceptualization/flows/`):
...
**Implementation flows** (`dev-implementation/flows/`):
...
```jsonc
{
  ...
  "next_flows": [                    // suggested follow-on flows shown at completion
    { "id": "audit", "domain": "dev-quality", "label": "Audit →", "hint": "Run static analysis." }
  ]
}
```
...
```jsonc
{
  ...
  "data": {
    ...
    // See dev-shared/contracts/feedback_loop.md for the full protocol.
...
See `dev-shared/agent-config.json` for standalone_mode settings.
