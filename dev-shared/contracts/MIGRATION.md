...
| Phases taxonomy | Named objects with sub-phases and orders | Simple arrays of step IDs | **Remains in flows** as group nodes + `dev-shared/contracts/flows.md` |
...
| Global runtime config | `config` section (standalone_mode, grounding_mode, standards_mode) | Not present | **Moved to `dev-shared/agent-config.json`** |
...
| `config.standalone_mode` | `dev-shared/agent-config.json` |
| `config.grounding_mode` (global) | `dev-shared/agent-config.json` |
| `config.grounding_mode.step_folders` | flow node `data.grounding_folder` |
| `config.standards_mode` | `dev-shared/agent-config.json` |
| `config.orchestrator_skill` | `dev-shared/agent-config.json` |

### New files created

| File | Purpose |
|---|---|
| `dev-shared/agent-config.json` | Global runtime config (standalone_mode, grounding, standards, orchestrator IDs) |
| `dev-shared/contracts/flows.md` | Contract explaining the flow system, how to read/write flows |
| `dev-conceptualization/flows/flow.schema.json` | Updated JSON schema with writes, requires, user_inputs, feedback, grounding_folder |
| `dev-conceptualization/flows/mvp.json` | Reference implementation — fully enriched with all new node fields |
| `dev-shared/contracts/skill_template.md` | Updated — removed all pipeline.json references, uses flow node fields |
...
| REFERENCES point to `shared/contracts/` (legacy path) | REFERENCES point to `dev-shared/contracts/` | Updated to monorepo structure |
| Enforcement script: `shared/scripts/validate_skill_rules.py` | `dev-shared/scripts/validate_skill_rules.py` | Updated path |
...
| Expert Discovery | `prog-expert-*` skill naming → `dev-implementation-expert-<tech>` | Matches actual monorepo skill directory names |
| Path refs | `05_techstack/stack.md`, `03_features/` | `3_blueprint/1_techstack/stack.md`, `2_experience/2_features/` |
| `cf__shared/` refs | `cf__shared/` | `dev-shared/` |
...
| REFERENCES path | `shared/contracts/` | `shared/contracts/` | **`dev-shared/contracts/`** |
| Script path | n/a | `../shared/scripts/` | **`dev-shared/scripts/`** (4 levels up) |
...
| `ui_components.md` | Not present | Present (PostXL catalog) | **Not included** — stack-specific; belongs in dev-standards/profiles |
...
| Stack defaults | Inline multi-stack blocks (Directus/Supabase/Nuxt/NestJS) | `references/postxl_defaults.md` (PostXL only) | **Neither** — agent derives from stack.md; details in dev-standards/profiles |
...
| Stack selection | Dynamic — scans `dev-standards/profiles/*/SKILL.md` | Fixed — hardcoded PostXL/NestJS/Next.js | **CF approach** — dynamic discovery at runtime |
...
**From CF:**
- Dynamic stack discovery (scans `dev-standards/profiles/*/SKILL.md` at runtime)
...
| Setup templates | Not present | PostXL-specific templates/ | **Not included** — templates belong in dev-standards/profiles |
...
|---|---|---|---|
| Scaffold approach | Generic — reads tech stack profile | PostXL-specific — `pxl create-project --schema postxl-schema.json` | **Profile-based** — reads dev-standards/profiles/<tech_stack_skill>/SKILL.md |
...
| Skill | Change |
|---|---|
| `migrate` | Canonical paths: `3_blueprint/3_datamodel/`, `3_blueprint/1_techstack/`, `dev-shared/contracts/`. Expert skills from `dev-implementation-experts-*`. |
