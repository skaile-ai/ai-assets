  ...
  user_inputs:
    dialog:
      - id: "scope"
        ...
        required: true
        default: "smoke"
        hint: "'smoke' = quick pass through main pages; 'all' = comprehensive check of every area"
      - id: "base_url"
        label: "Frontend URL"
        type: "text"
        required: false
...
STEP 3: Execute verification based on scope
  IF scope = "smoke"
    - Run abbreviated version of each area (load page, check it renders, move on)
    - Read references/dashboard-and-navigation.md -> run SMOKE section only
    - Read references/project-creation.md -> run SMOKE section only
    - Read references/settings.md -> run SMOKE section only
  IF scope = "all"
    - Run every reference file in order: dashboard, project-creation, workspace, settings, admin
  IF scope = specific area
    - Read references/{scope}.md and execute all checks
...
NEVER send real messages to the agent backend -- only verify UI structure exists
