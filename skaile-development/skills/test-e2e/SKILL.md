...
WRITES
  L5 Playwright (forge apps, platform/e2e extension):
    ...
    <target>/tests/e2e/global-setup.ts              — if missing (forge/L5-concept uses test/e2e/)
...
MUST  pick a dedicated port per forge app to avoid conflicts (forge/L5-concept uses 3344; assign unique ports — see the port allocation table at the bottom of this file)
...
STEP 3a (web, skip if mode=generate): Scaffold Playwright (L5)
  ...
  Test-file suffix is `.spec.ts` (Playwright default) — this distinguishes L5 browser specs from L4 Vitest `.test.ts`. The entry path is `tests/e2e/` for new forge apps; only `forge/L5-concept` uses `test/e2e/`.
  ...
  Add `.gitignore` entries if missing: `playwright-report/`, `test-results/`, `tests/e2e/screenshots/` (or `test/e2e/screenshots/` for forge/L5-concept).
...
- **Writes:**
  ...
  - L5 Playwright: `<target>/playwright.config.ts`, `<target>/tests/e2e/**` (or `test/e2e/**` for forge/L5-concept)
