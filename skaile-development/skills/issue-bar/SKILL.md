---
name: "issue-bar"
description: "[skaile-development] The bar a ticket must clear to exist: six impact
  questions (user harm, money, stability, security, team drag, significant benefit),
  each answered with evidence. Use before filing an issue an agent found on its own (a ship follow-up,
...
uptime, to security or to the team's ability to ship, or a **significant gain** that
doing the work buys. A true observation about the code is neither. "This could be more robust", "a second check would be
...
These kinds of ticket answer NO on all six unless they carry specific evidence that
...
- **Cleanup.** Refactors, renames, dead code, consistency, symmetry ("A validates,
  so B should too"), comment and docs polish. Q6 is YES only when the cleanup buys a
  gain you can name, such as unblocking a specific piece of planned work.
- **Micro-optimisation.** A faster path nobody waits on. Q6 is YES only when the
  current cost is measured on a path users or jobs actually hit.
...
2. Answer the six questions against the **current** code and data, not what the
...
Closing against the issue bar (all six impact questions answered NO):
- User harm: <…>
- Money: <…>
- Stability: <…>
- Security: <…>
