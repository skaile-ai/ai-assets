...
> "There are N stale page(s). Run `skaildev-doc` in `update` mode to bring them back in sync."

**If high priority gaps exist:**
> "There are N high priority gap(s) (undocumented public API or @doc:important files). Run `skaildev-doc` in `write` mode to address these."
