...
description: >
  ...
  Supersedes skaildev-update-docs.
metadata:
  version: "1.0.0"
  tags:
    - documentation
...
# skaildev-doc — Skaile Dev Documentation Skill
...
- When `skaildev-implement` finishes — it calls this skill in `update` mode automatically
...
- For writing `_devlog` entries directly — this skill delegates to `skaildev-devlog`
...
MUST  delegate `_devlog` entries to `skaildev-devlog` — never write devlog content directly
...
Run the appropriate helper script from `ai-resources/skaile-development/skills/skaildev-doc/scripts/`
...
- **Called by:** `skaildev-implement` after tests pass (in `update` mode)
- **Calls:** `skaildev-devlog` — after any write or update operation, delegate devlog entry creation
- **Supersedes:** `skaildev-update-docs` — this skill covers all modes that the old skill handled,
  plus `write`, `audit`, and `status` modes. Do not use `skaildev-update-docs` for new work.
