...
skaile skill run skaildev-doc --input MODE=status

# Check a specific package
skaile skill run skaildev-doc --input MODE=status --input SCOPE=agent-framework/cli
```
...
skaile skill run skaildev-doc --input MODE=update --input SCOPE=forge/common-backend
```
...
**Typical use:** After `skaildev-implement` finishes (it calls this automatically) or when a developer has made code changes that affect public behavior.
...
skaile skill run skaildev-doc --input MODE=audit

# Audit a specific package
skaile skill run skaildev-doc --input MODE=audit --input SCOPE=agent-framework/resolver
```
...
skaile skill run skaildev-doc --input MODE=write --input TARGET=agent-framework/cli

# Write docs for specific source files
skaile skill run skaildev-doc --input MODE=write --input TARGET=forge/project/src/auth.ts

# Write docs for a feature or topic
skaile skill run skaildev-doc --input MODE=write --input TARGET="session lifecycle"
```
...
```bash
# Check entire monorepo
bun ai-resources/skaile-development/skills/skaildev-doc/scripts/doc-status.ts --root .

# Check a specific package with markdown output
bun ai-resources/skaile-development/skills/skaildev-doc/scripts/doc-status.ts \
  --root . \
...
bun ai-resources/skaile-development/skills/skaildev-doc/scripts/doc-audit.ts --root .

# Audit a specific package
bun ai-resources/skaile-development/skills/skaildev-doc/scripts/doc-audit.ts \
  --root . \
...
bun ai-resources/skaile-development/skills/skaildev-doc/scripts/doc-tracker.ts --root .

# Track a specific package
bun ai-resources/skaile-development/skills/skaildev-doc/scripts/doc-tracker.ts \
  --root . \
...
The `skaildev-implement` skill automatically calls `skaildev-doc` in `update` mode after tests pass and verification succeeds. This ensures documentation stays in sync with code changes.

**Why it matters:** You do not need to manually run `skaildev-doc` after a feature is implemented — the orchestrator handles it.
...
1. **Run `skaildev-implement`** to scaffold and build the feature.
2. **Run tests and verification** to ensure everything works.
3. **`skaildev-implement` calls `skaildev-doc update` automatically.**
4. *Done.* Documentation is now in sync.

If you need to write docs for undocumented code, run:

```bash
skaile skill run skaildev-doc --input MODE=write --input TARGET=<package-or-file>
```
...
```bash
skaile skill run skaildev-doc --input MODE=write --input TARGET=agent-framework/runner
```
...
1. **Run audit** to catch gaps early:
   ```bash
   skaile skill run skaildev-doc --input MODE=audit --input SCOPE=<your-package>
   ```
...
- **`skaildev-implement`** — calls `skaildev-doc update` automatically after implementation succeeds
- **`skaildev-devlog`** — record what was documented in the project devlog (called after write/update)
