...
Entries here are typically registered in the catalog via `skaile resource add <github-url>` rather than stored as local files. The `external/` folder may contain symlinks, stubs, or reference notes pointing to the canonical external source.
...
```bash
skaile resource add https://github.com/org/external-skills external-skills
```

## Conventions

- Do not copy external files into this folder — reference them via `skaile resource`
