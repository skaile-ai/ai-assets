...
Entries here are typically registered in the `arm` catalog via `arm resource add <github-url>` rather than stored as local files. The `external/` folder may contain symlinks, stubs, or reference notes pointing to the canonical external source.
...
- Do not copy external files into this folder — reference them via `arm resource`
