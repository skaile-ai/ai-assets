...
- **A date before the workbook's serial epoch is silently stored as text and reported as success** ([#77](https://github.com/skaile-ai/excel-mcp/issues/77)). Excel counts dates from 1899-12-31 — or 1904-01-01 in a 1904-windowed workbook — and `range.set` cannot represent anything earlier. Rather than refusing, it writes the ISO string into a text cell: `range.get` then answers `type: "string"`, so the value still looks right in the cell but sorts and filters as text. Measured on v0.4.0: `1899-12-31` round-trips correctly as a date, `1850-06-15` comes back a string. **Check the returned type after writing a historical date**, or keep pre-epoch dates in a column Excel does not have to treat as dates.
