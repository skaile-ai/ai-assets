...
|---|---|---|---|
| [excel](xls/MCP.md) | `mcp/xls` | alpha (v0.5.0) | Excel (.xlsx/.xlsm/.xls) read/write via Apache POI, 55 tools over stdio — headless recalculation, cell styling & sheet presentation (write and read-back), conditional formatting (write and read-back), data validation, merged regions, cell notes and threaded comments, outline grouping, print setup, formula pattern search, line/bar charts, per-workbook agent context, multi-call `batch`, a read-only stream mode for workbooks too large to load, plus a review path (`workbook.audit`, `cell.trace`). |
