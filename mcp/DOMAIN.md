...
├── docx/              <- word catalog entry (code: skaile-ai/word-mcp)
│   └── MCP.md         <- catalog manifest + guidance for mcp:word resolution
...
|---|---|---|---|
| [excel](xls/MCP.md) | `mcp/xls` | alpha (v0.4.0) | Excel (.xlsx/.xlsm/.xls) read/write via Apache POI, 55 tools over stdio — headless recalculation, cell styling & sheet presentation (write and read-back), conditional formatting (write and read-back), data validation, merged regions, cell notes and threaded comments, outline grouping, print setup, formula pattern search, line/bar charts, per-workbook agent context, multi-call `batch`, a read-only stream mode for workbooks too large to load, plus a review path (`workbook.audit`, `cell.trace`). |
...
| [word](docx/MCP.md) | `mcp/docx` | stable (v0.7.0) | Word (.docx) source-free creation, Template composition, and in-place editing via Apache POI XWPF — a neutral blank baseline plus Block-addressed body/header/footer/text-box flows, Named styles, tables, PDF previews, properties/fields, image/media writes, and Revision/Comment collaboration; 38 tools over stdio. |
...
- **Catalog entries describe the released version.** An `MCP.md` body describes the release its `version:` names — not the server's `main`. The platform flake pins each locally-built MCP to a release tag, so every body claim (tool inventory, per-category counts, Limitations, `Known issues in <version>` headings) belongs to that release. Describing unreleased work would tell an agent about tools the jar in its session does not have — a wrong document, not merely a stale one.
- **Bump-PR review duties.** The `mcp-release` dispatch rewrites only the `version:` marker line; it touches nothing in the body. Tool counts, per-category counts, `Known issues in <version>` headings and any Limitation the new release invalidates must be re-derived from that release's README tool table before the bump PR merges — as must the server's row in the table above. The workflow attaches a generated checklist of every version-sensitive line it found and opens the PR as a **draft**, so a bump cannot be waved through while the prose still describes the previous release; clear the draft once it matches.
- **Declaring an enumeration for the parity check.** An entry may wrap an enumeration whose backticked names are the server's **wire** names — `workbook.open`, not a prose paraphrase — in a `<!-- mcp-catalog-tools -->` / `<!-- /mcp-catalog-tools -->` pair. Both markers are required: the block's extent is declared, never inferred from the next heading, because the prose below an enumeration names tools in passing and a stray mention of a tool the release **dropped** would satisfy the diff and hide the removal. Declaring it opts that entry into a mechanical, name-level diff against the `tools` array the `mcp-release` dispatch carries — which is all this repo ever sees; how the sender builds that array is its own business — and the bump PR body then lists exactly which tools the release added and which it dropped. The marker is an opt-in and not a heuristic: an entry that omits names for whole categories, or writes them in a form the wire does not use, would diff to dozens of differences that are not there — so an entry without the marker is reported as **not checked** rather than guessed at, and that wording is deliberate, because "not checked" and "checked, nothing changed" must never read the same. Eagerness stays prose: the check prints the release's eager tools beside a pointer to the paragraph that claims them, for a human to compare, and asserts nothing about it. `mcp/xls/MCP.md` is the first entry to declare the marker.
