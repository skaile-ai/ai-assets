...
## Goal
Expert-level implementation of shadcn-vue in Nuxt projects. Covers the `shadcn-nuxt` module, `components.json` registry configuration, Tailwind CSS v4 integration via `@tailwindcss/vite`, variant composition with `class-variance-authority`, utility merging with `tailwind-merge` + `clsx`, and auto-imported shadcn component patterns.

## Core Workflow (Progressive Disclosure)
1. **Context Analysis**: Analyze `components.json`, `nuxt.config.ts`, `tailwind.config.js`, and existing shadcn component usage.
...
- Scope: shadcn-vue and Tailwind CSS integration only. For Nuxt core concerns use `prog-expert-nuxt`. For VueUse composables use `prog-expert-vueuse`.

## Key Technologies
- `shadcn-nuxt`, `shadcn-vue`
...
- **Recipes** (Larger patterns like form composition, CVA variants, theming):
