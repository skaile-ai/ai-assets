...
building_blocks:
  ...
  skills: 'One skill per service: exa (AI search), perplexity (research via OpenRouter), searxng (local privacy search), context7-api (library docs), docling (document conversion), elevenlabs (TTS), outline (wiki API), outline-cli (wiki CLI), revealjs (presentation decks, zero-install), slidev (presentation decks, markdown).'
  agents: 'TBD'
  prompts: 'TBD'
  tools: 'TBD'
stage: alpha
