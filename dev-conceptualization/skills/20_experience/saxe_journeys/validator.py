#!/usr/bin/env python3
"""Auto-generated validator for concept-2-experience-1-journeys.
Re-generate with: /compile-validators concept-2-experience-1-journeys
"""
import sys
...
SKILL = "concept-2-experience-1-journeys"
STORIES = "_concept/2_experience/1_journeys/stories.json"
SCHEMA = "skills/shared/contracts/stories_schema.json"
...
def validate(cwd: str) -> dict:
    ...
    # MUST produce exactly one hero story map
    ...
    def check_ears():
        ...
        for i, s in enumerate(stories):
            ...
            if not ac:
                return False, f"Story '{s.get('id', i)}' has no acceptance criteria"
            for j, criterion in enumerate(ac):
                if isinstance(criterion, dict) and criterion.get("kind") != "ears":
                    continue  # non-EARS criteria are allowed alongside
        return True, ""

    v.must("write EARS acceptance criteria for every story", check_ears)

    # MUST derive personas from brief audience and research
    ...
    # NEVER write outside 2_experience/1_journeys/
    v.never("write outside 2_experience/1_journeys/", lambda: (
        v.no_files_matching("_concept/2_experience/2_features/**/*.md")
        if not list(v.glob_files("_concept/2_experience/2_features/**/*.md"))
        else (True, "")  # features may pre-exist from a prior step
    ))

    # NEVER define more than one hero story map
    ...
    # Exactly one story map has stage: hero (same as MUST rule, but in checklist)
    ...
    # Priority distribution: hero=must, backlog=could/wont
    ...
    # Semantic checks — skip
    v.skip("Summary table shown and approved by user", rule_type="CHECKLIST",
           reason="process — cannot verify user interaction")
