...
The model uses **semantic types** (from `dev-shared/contracts/semantic_types.md`), not
...
**REQUIRED BACKGROUND:** Read `dev-shared/contracts/concept_structure.md`,
`dev-shared/contracts/semantic_types.md`, `dev-shared/contracts/golden_principles.md`,
`dev-shared/contracts/feedback_loop.md`, and `dev-shared/contracts/seed_data.md`.
...
REFERENCES
  dev-shared/contracts/concept_structure.md   — valid _concept/ paths and naming rules
  dev-shared/contracts/semantic_types.md      — stack-independent field type catalog
  dev-shared/contracts/golden_principles.md   — naming rules: snake_case fields, PascalCase enums, _id suffix
  dev-shared/contracts/feedback_loop.md       — cross-reference protocol (features ↔ datamodel)
  dev-shared/contracts/seed_data.md           — scenario format and data quality rules
  references/model_conventions.md             — DBML + model.json template, naming rules, feature_map format

MUST  use semantic types from dev-shared/contracts/semantic_types.md — not SQL types
...
STEP 5: Write seed data
  - Read dev-shared/contracts/seed_data.md for scenario format and quality rules
  ...
  OUTPUT _concept/3_blueprint/3_datamodel/seed.json
    See dev-shared/contracts/seed_data.md for full format and examples.
...
STEP 8: Human approval
  CHECKPOINT datamodel_approved
    ...
    >
    > Technical summary:
    >   Entities: N, Relationships: N, Enums: N, Features updated: N
    >
...
STEP 9: Stack translation (on request only)
  IF user asks "generate Prisma schema", "export to Directus", "SQL DDL", etc.:
    - Read stack.md to confirm the target
    - Use semantic_types.md translation table
    - Write stack-specific output (separate file, not a replacement for model.json)
