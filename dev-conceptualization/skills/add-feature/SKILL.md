...
`dev-shared/contracts/feedback_loop.md`, and `dev-shared/contracts/golden_principles.md` before proceeding.
...
REFERENCES
  dev-shared/contracts/concept_structure.md      — valid paths, naming rules
  dev-shared/contracts/frontmatter.md            — required YAML fields per file type
  dev-shared/contracts/feedback_loop.md          — cross-reference protocol
  dev-shared/contracts/golden_principles.md      — entity naming, numbering rules
  dev-shared/contracts/semantic_types.md         — stack-independent field types
...
STEP 4: Write feature spec
  IF new feature
    - If fits existing group, add file to that group folder
    - If new group needed, create next sequential numbered folder (NN_group_name)
...
STEP 7: Optional implementation (if app already built)
  IF _implementation/progress.json exists
    > "The app has an existing implementation. Do you want to implement this feature now,
    > or save it for the next implementation run?"
