...
| Range I/O | `range.set_style` | `handle`; `sheet`; `range` (bounded A1, no full-column/row); `style` (object: `fill_color` `"#RRGGBB"`, `font` {name,size,bold,italic,color}, `border` {top,bottom,left,right → {style,color?}}, `number_format`, `horizontal_alignment`, `vertical_alignment`, `wrap_text`) | Applies cell formatting to every cell in the range. Each attribute is optional and **merges** onto the cell's existing style, so omitted attributes are kept (set only a fill and any existing number format survives). Empty cells are materialised so the style takes effect. Styles and fonts are deduplicated, so a uniform range costs one style and never nears Excel's 64k-style cap. `.xlsx`/`.xlsm` only — `.xls` fails with STYLE_INVALID. In-memory until workbook.save. |
| Sheet management | `sheet.set_format` | `handle`; `sheet`; `column_widths?` ([{column,width}], width in char units); `row_heights?` ([{row,height}], 1-based row, height in points); `freeze?` ({rows,cols} — header freeze is {1,0}, {0,0} unfreezes); `tab_color?` (`"#RRGGBB"`) | Sets sheet-level presentation: column widths, row heights, a frozen header pane, and the worksheet tab color. Every group is optional; only the supplied groups change. `.xlsx`/`.xlsm` only — `.xls` fails with STYLE_INVALID. In-memory until workbook.save. |
...
- Tool surface: 28 tools across workbook lifecycle, range I/O (incl. cell styling via `range.set_style`), sheet management (incl. presentation via `sheet.set_format`), row/col mutation, tables, named ranges, and read-only VBA. No charts, no pivots, no conditional formatting, no merged-region writes, no Power Query / DAX. Styling is `.xlsx`/`.xlsm` only.
