...
**Phase 10 — Acceptance + polish.** Walk every §13 acceptance criterion; fix gaps. Write the README. Confirm `excel-mcp-server-future-work.md` was created and seeded per §1.6. Add `WorkbookListHandlesTool` (see §9.1 for the contract) — a diagnostic tool that returns all currently open workbook handles so an agent can self-audit after error-recovery paths where a prior handle may have been lost; this tool is registry-only and imports no POI. Rename the `scripts/` directory to `smoke/` — it currently contains only phase-smoke and regression-smoke shell tests, so the generic `scripts/` name is misleading; reserving `scripts/` keeps a clean home for future operational scripts (startup wrappers, healthcheck probes, container-build helpers). Update the two path references to smoke scripts under "Plan deferrals during implementation" in `excel-mcp-server-future-work.md`, plus any references that may have accumulated in the README or newer deferrals by the time Phase 10 runs. Audit all 26 tool descriptions and parameter descriptions against the authoring template documented in `excel-mcp-server-future-work.md` §"Authoring conventions" (three-part effect / prerequisite / gotcha body; example + constraints + units on every parameter; no engine-layer leakage). Resolve any Phase 10 stop-gate items listed in the future-work doc. Final `mvn verify` clean; Docker image builds and starts under 5 seconds.
