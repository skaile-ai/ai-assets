...
- **`sheet.delete` on the last remaining sheet succeeds and `workbook.save` then produces a file Excel refuses to open.** (Phase 6 manual testing, `SheetDeleteTool` / `PoiAtomicSaver`) Excel rejects a workbook with zero sheets as corrupt — the inverse of the Phase-4 seeding fix on `workbook.create`. v1 does not prevent the delete or the save: the tool description warns ("Excel rejects a workbook with zero sheets as corrupt — always keep at least one sheet") but the warning is honest rather than enforced. Reproduced: create workbook with only "Sheet1", `sheet.delete Sheet1`, `workbook.save` → file on disk is an OOXML package with zero `<sheet>` entries, Excel 365 refuses to open it with "The file is corrupt and cannot be opened." Chose: document and leave to the agent for v1, same way we leave many invariants to the caller. Future better: either auto-reject the last-sheet delete with a new code (e.g. `SHEET_LAST_REMAINING`), or auto-seed a replacement sheet (`"Sheet1"`) before the delete commits. Decision likely falls to Phase-10 acceptance audit.
