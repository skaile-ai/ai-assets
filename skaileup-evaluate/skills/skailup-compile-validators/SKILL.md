---
name: "skailup-compile-validators"
...
REFERENCES
  skaileup-shared/scripts/validator_lib.py        — shared validation library (read for full API)
  skaileup-shared/contracts/skill_grammar.md      — MUST/NEVER/CHECKLIST DSL keywords
...
STEP 4: Generate validator.py
  ...
  **Determining sys.path depth:**
  ...
  OUTPUT <skill-dir>/validator.py
    ...
    sys.path.insert(0, str(Path(__file__).resolve().parents[N] / "skaileup-shared" / "scripts"))
